...
class DecisionTree:
    ...
    def _traverse_tree_proba(self, x, node):
        if node.is_leaf_node():
            return self._leaf_proba(node.value)

        if x[node.feature] <= node.threshold:
            return self._traverse_tree_proba(x, node.left)
        return self._traverse_tree_proba(x, node.right)

    def _leaf_proba(self, value):
        # Convert the value to a list before passing it to Counter
        class_counts = Counter([value])
        total_samples = sum(class_counts.values())
        probs = {cls: count / total_samples for cls, count in class_counts.items()}
        return probs
...
def random_forest_train():
    dataset = pd.read_csv('Breast-Cancer-Predictor-master/Breast Cancer Data.csv')
...
def random_forest_test(clf):
    t = time()
    output = clf.predict(X_test)
    acc = accuracy(y_test, output)
    print("The accuracy of testing data from random forest is: ", acc)
    print("The running time from random forest is: ", time() - t)
...
def random_forest_predict(clf, inp):
    ...
    print("The running time from random forest: ", time() - t)
